...
Under one-sided alternatives in the additive model, a phase transition in the $r$-$\beta$ plane also exists for the exact support recovery problem. The boundary the phase transition was found to be
\begin{equation} \label{eq:strong-classification-boundary-additive}
    \widetilde{g}(\beta) = \left(1 + \sqrt{1-\beta}\right)^2,
\end{equation}
which is strictly below that of two-sided alternative case in \eqref{eq:strong-classification-boundary-chisquared}, except when the problem is extremely sparse ($\beta = 1$).
In particular, as the problem becomes denser ($\beta\to0$), the required signal size in the one-sided additive model $\widetilde{g}(\beta)$ tends to $4$, while the required signal size in the chi-square (or two-sided alternative additive) model ${g}(\beta)$ tends to $\to3+2\sqrt{2}\approx 5.828$, almost 50\% higher.
...
In simulations, we find the asymptotic predictions to be quite accurate when $\nu>3$, even in moderate dimensions. 
For $\nu\le3$, the phase transitions take place somewhat below ${g}$, and closer to $\widetilde{g}$.
