...
$$
Therefore, we can pick $q>1$ so small such that 
$$
...
\begin{equation*}
    ...
    {Z_1^2 + \ldots + Z_{\nu-1}^2 + (Z_\nu + \sqrt{\overline{\Delta}})^2}.
\end{equation*}
Dividing by $u_p$, and taking minimum over $S$, we obtain
\begin{align}
    \frac{m_S}{u_p} =& \min_{i\in S} \frac{\chi_\nu^2(\lambda(i))}{u_p} 
    \stackrel{\mathrm{d}}{\le} \min_{i\in S} \frac{\chi_\nu^2(\overline{\Delta})}{u_p} \\
    \stackrel{\mathrm{d}}{\le}& \min_{i\in S}\frac{Z_1^2(i) + \ldots + Z_{\nu-1}^2(i) + Z_\nu^2(i)}{u_p} + \min_{i\in S}\frac{2\sqrt{\overline{\Delta}}Z_\nu(i)}{u_p} + \frac{\overline{\Delta}}{u_p} \\
    \stackrel{\P}{\to}& \;0 - 2\sqrt{\overline{r}}\sqrt{1-\beta} + \overline{r}. \label{eq:chi-square-necessary-3}
\end{align}
...
\begin{enumerate}
    ...
    $$ F^{i}_2(t) \le F^{i}_1(t), \quad \text{for all} \quad i\in S.$$
\end{enumerate}
If we apply the BH procedure at the same nominal level of FDR $\alpha$, then the FNR under Alternative 2 is bounded above by that under Alternative 1.
\end{lemma}
