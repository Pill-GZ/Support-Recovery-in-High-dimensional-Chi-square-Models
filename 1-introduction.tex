...
Studying model \eqref{eq:model-chisq} will allow us to tackle the omnidirectional alternatives, which are arguably the more realistic in discovery sciences where little to no prior knowledge is available.
In comparing with results on one-sided alternatives, we will also be able to quantify if, and how much of a price has to be paid for the additional uncertainty of the two-sided alternatives in the additive model \eqref{eq:model-additive}.
...
Under appropriate parametrizations of the signal sizes and sparsity levels in $\lambda$, the results establish the phase transitions of support recovery problems in the chi-square model, previewed in Figure \ref{fig:phase-chi-squared}.
They also reveal that the exact support recovery problem can be substantially harder under the omnidirectional alternatives, while the difficulties of approximate support recovery problems remain unchanged.
...
\begin{figure}
      ...
      \caption{The phase diagram of the high-dimensional chi-square model \eqref{eq:model-chisq}, illustrating the boundaries of the exact support recovery problem (Theorem \ref{thm:chi-squred-strong-boundary}), approximate support recovery problem (Theorem \ref{thm:chi-squred-weak-boundary}), and the signal detection problem (described in \citep{donoho2004higher}). 
      ...
      In comparison, for one-side alternatives in the additive model \eqref{eq:model-additive} under Gaussian noise, the strong classification boundary (dashed line) can be substantially lower than that in the chi-square model; the weak classification and detection boundaries are identical to those in the chi-square models .} 
      \label{fig:phase-chi-squared}
\end{figure}


The notion of signal sizes in the chi-square models, unlike in additive error models, is perhaps not as easy to interpret.
Our third contribution demystifies this notion of signal sizes in the context of association tests.
...
Practical issues are also addressed to make for simple and effective power analysis in such problems.
