...
Under appropriate parametrizations of the signal sizes and sparsity levels in $\lambda$, these results establish the phase transitions of support recovery problems in the chi-square model, previewed in Figure \ref{fig:phase-chi-squared}.
They also reveal that exact support recovery problems can be substantially harder under the omnidirectional alternatives, while the difficulties of approximate support recovery problems remain unchanged.
...
\begin{figure}
      ...
      \caption{The phase diagram of the high-dimensional chi-square model \eqref{eq:model-chisq}, illustrating the boundaries of the exact support recovery problem (top solid curve; Theorem \ref{thm:chi-squred-strong-boundary}), approximate support recovery problem (middle solid curve; Theorem \ref{thm:chi-squred-weak-boundary}), and the signal detection problem (lower solid curve; described in \citep{donoho2004higher}). 
      ...
      In comparison, for one-side alternatives in the additive model \eqref{eq:model-additive} under Gaussian noise, the strong classification boundary (dashed curve) can be substantially lower than that in the chi-square model; the weak classification and detection boundaries are identical to those in the chi-square models .} 
      \label{fig:phase-chi-squared}
\end{figure}


The notion of signal sizes in the chi-square models, unlike in additive error models, is perhaps not as transparent.
